import { BOOKING_URL, LINKEDIN_URL } from "../config";

// Case-study pages render this brand-only (actions={false}): their booking CTA
// lives in the sticky case-study panel, and the nav actions would collide with
// the fixed Close button on mobile. The brand then links "/" so every project
// page has a real crawlable anchor back home.
export default function Header({
  brandHref = "#top",
  actions = true,
}: {
  brandHref?: string;
  actions?: boolean;
}) {
  return (
    <nav className="nav" aria-label="Primary">
      <a className="nav__brand" href={brandHref} aria-label="Gjorgi Krmzov, home">
        [GK]
      </a>

      {actions && (
        <div className="nav__actions">
          <a
            className="btn btn--sm"
            target="_blank"
            href={BOOKING_URL}
            data-cursor="Let's talk"
          >
            Book a call
          </a>
          <a
            className="nav__social"
            href={LINKEDIN_URL}
            target="_blank"
            rel="noreferrer"
            aria-label="LinkedIn"
            data-cursor="Connect"
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.48-.9 1.64-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.45v6.29zM5.34 7.43a2.06 2.06 0 1 1 0-4.13 2.06 2.06 0 0 1 0 4.13zM7.12 20.45H3.56V9h3.56v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.22.79 24 1.77 24h20.45c.98 0 1.78-.78 1.78-1.73V1.73C24 .77 23.2 0 22.22 0z" />
            </svg>
          </a>
        </div>
      )}
    </nav>
  );
}
